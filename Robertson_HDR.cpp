...
int main(int argc, char** argv )
{
	if(argc != 4)
	{
		printf("usage: ./Robertson_HDR #Pics threshold <Image_Folder>\n");
		return -1;
	}
	...
	for(int i = 0; i < num_pics; i++)
	{
		...
		string img_Name = img_folder + "img" + tmp + img_type;
	...
	for(int cBGR = 0; cBGR < 3; cBGR ++)
	{
		...
		for(int T = 0; T < 30; T++)
		{
			...
			for(int r = 0; r < rows; r++)
			{
				for(int c = 0; c < cols; c++)
				{
					...
					for(int t = 0; t < num_pics; t++)
					{
						...
						est_x[r][c] += weight(bit8) * times[t] * Im[cBGR][bit8];
						wsum += weight(bit8) * times[t] * times[t];
					}
			...
			for(int r = 0; r < rows; r++)
			{
				for(int c = 0; c < cols; c++)
				{
					for(int t = 0; t < num_pics; t++)
					{
						...
						cardI[bit8] ++;
						Im[cBGR][bit8] += times[t] * est_x[r][c];
					}
				}
			}
			...
			for(int i = 0; i < 256; i++)
			{
				if(cardI[i] == 0) continue;
				Im[cBGR][i] /= cardI[i];
			}
			float mid = Im[cBGR][128];
			for(int i = 0; i < 256; i++)
				Im[cBGR][i] /= mid;
		}
	}
	...
	for(int cBGR = 0; cBGR < 3; cBGR ++)
	{
		for(int r = 0; r < rows; r++)
		{
			for(int c = 0; c < cols; c++)
			{
				...
				{
					int t = best_expo[disc_fa[find(r * cols + c)] - 1];
					int bit8 = image[t].at<Vec3b>(r, c)(cBGR);

					altern = Im[cBGR][bit8] / times[t];
				}
				...
				for(int t = 0; t < num_pics; t++)
				{
					...
					Isum += weight(bit8) * times[t] * Im[cBGR][bit8];
					wsum += weight(bit8) * times[t] * times[t];
				}
				...
				if(disc_fa[find(r * cols + c)] != 0)
				{
					...
					HDR.at<Vec3f>(r, c)[cBGR] = alpha * altern + (1 - alpha) * Isum / wsum;
				}
				else
					HDR.at<Vec3f>(r, c)[cBGR] = Isum / wsum;
			}
		}
	}
	imwrite("HDR_Rob_image.exr", HDR);
	return 0;
}
